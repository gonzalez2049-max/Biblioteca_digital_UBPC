...
function ensureSchema() {
  ...
  schemaReady = (async () => {
    ...
    await sql`INSERT INTO singletons (key, data) VALUES ('branding', '{}') ON CONFLICT (key) DO NOTHING`;
    await sql`INSERT INTO singletons (key, data) VALUES ('gaps', ${JSON.stringify({ items: [] })}) ON CONFLICT (key) DO NOTHING`;
    await sql`INSERT INTO singletons (key, data) VALUES ('featured', ${JSON.stringify({ titulo: 'Lo esencial del turno', ids: [] })}) ON CONFLICT (key) DO NOTHING`;
    await sql`INSERT INTO singletons (key, data) VALUES ('faq', ${JSON.stringify({ items: [] })}) ON CONFLICT (key) DO NOTHING`;
    await sql`INSERT INTO singletons (key, data) VALUES ('news', ${JSON.stringify({ activo: false, titulo: '', texto: '', enlace: '' })}) ON CONFLICT (key) DO NOTHING`;
...
const DEFAULT_STATS = {
  ...
  aperturas: {}, busquedas: [], sinResultado: [], usoTema: {}, usoRol: {}, utilidad: {}, progreso: {}, interacciones: [],
};
