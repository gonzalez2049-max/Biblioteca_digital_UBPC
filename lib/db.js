// Acceso a la base de datos (Vercel Postgres) y creación/seed del esquema.
...
function ensureSchema() {
  ...
  schemaReady = (async () => {
    ...
    // Administrador principal inicial desde variables de entorno (solo si no hay ninguno).
    const { rows } = await sql`SELECT count(*)::int AS n FROM admins`;
    if (rows[0].n === 0) {
      const email = process.env.ADMIN_EMAIL || 'admin@huap.cl';
      const pass = process.env.ADMIN_PASSWORD || 'huap-cambiar';
      const hash = bcrypt.hashSync(pass, 10);
      await sql`INSERT INTO admins (id, nombre, correo, rol, activo, password_hash, color)
        VALUES ('u1', ${process.env.ADMIN_NAME || 'Administrador principal'}, ${email}, 'principal', true, ${hash}, '#7c5cfc')`;
    }
  })();
  return schemaReady;
}
